...
async def on_ready():
    ...
    await bot.change_presence(activity=discord.Activity(
        type=discord.ActivityType.watching, name="Kyle struggle with errors with Zion"))
        ...
        def search(query):
            ...
            with YoutubeDL({'format': 'bestaudio'}) as ydl:
                try:
                    ...
                    info = ydl.extract_info(f"ytsearch:{query}", download=False)['entries'][1]   
                else:
                    info = ydl.extract_info(query, download=False)
                    print(info)
                    nextPlaylistSong.append(info)
...
async def queue(ctx):
    ...
    for qNum, qTitle in enumerate(nextSong):
        if (qNum == 0):
