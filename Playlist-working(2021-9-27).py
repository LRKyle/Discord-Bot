...
async def hrand(ctx):
  ...
  print(f"The random doujin that the user got was {RandHentai}")
  Embed = discord.Embed(
  title = 'Randomize Hanime Page!',
  description = f'[:monkey:**Hanime randomized!**:monkey:](https://hanime.tv/browse/random?r={RandHentai})',
  color = discord.Color.from_rgb(101, 67, 33))
  Embed.set_image(url="https://c.tenor.com/Fl6m3zSv3XgAAAAd/monkey-spinning.gif")
  await ctx.channel.send(embed = Embed)
  ...
  if 'https://www.youtube.com/playlist?list=' in query:
    ...
    if 'entries' in info:
      for i in info['entries']:
        ...
        # if dur == 0:
        #   await ctx.send(f"**{i['title']}** is added to the queue!")
        #   songQueue.append(i['title'])
        #   songDuration.append(i['duration'])
        ...
        #   songDuration.pop(0)
        #   voice.play(source)
        #   await ctx.send(f"Now playing: **{i['title']}** 2")
        #   await ctx.send(dur)
        await ctx.send('lol')
    #Basically, make it the playlist is filtered into a list then play it normally
...
async def queue(ctx):
  ...
  for qNum,qTitle in enumerate(songQueue):
    durTime = int(songDuration[qNum])
    await ctx.send(f"`[{qNum + 1}] {qTitle} \n Duration: {convert(durTime)}`")
